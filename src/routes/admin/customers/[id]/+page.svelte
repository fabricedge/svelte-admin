...
<h1 class="text-2xl font-bold mb-6">Cliente</h1>
...
{:else}
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
    <div class="bg-white p-6 rounded-lg border border-gray-200">
      ...
      <div class="space-y-3 text-sm">
              ...
              <button
                onclick={async () => {
                  try {
                    ...
                    toast.success('Usuário rebaixado a CUSTOMER')
                  } catch (err: any) { toast.error(err.message) }
                }}
        ...
        <div>
          <p class="text-gray-500">Cliente desde</p>
  ...
  <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-200">
      <h2 class="font-bold">Histórico de Pedidos</h2>
    </div>
    {#if userData.orders.length === 0}
      <div class="p-6 text-sm text-gray-500">Nenhum pedido encontrado.</div>
    {:else}
      <table class="w-full text-sm">
        <thead class="bg-gray-50 border-b border-gray-200">
          <tr>
            ...
            <th class="text-right px-6 py-3 font-medium">Ações</th>
          </tr>
        </thead>
        <tbody>
          {#each userData.orders as order}
            <tr class="border-b border-gray-100 hover:bg-gray-50">
              ...
              <td class="px-6 py-3 font-medium {statusClass(order.status)}">{order.status}</td>
