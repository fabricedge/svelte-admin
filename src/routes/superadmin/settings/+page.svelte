<script lang="ts">
  ...
  async function handleMultiStoreToggle() {
    ...
    try {
      ...
      ctx.multiStoreEnabled = multiStoreEnabled
      toast.success(multiStoreEnabled ? 'Multi-store ativado' : 'Multi-store desativado')
    } catch (err: any) {
      toast.error(err.message)
    } finally {
...
{:else}
  <div class="space-y-10 max-w-lg">
    <!-- Multi-Store -->
    <section>
      <h2 class="text-lg font-semibold mb-4">Multi-Store</h2>
      <div class="rounded-lg bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-4">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-gray-900 dark:text-gray-100">Multi-store</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
              Permite que ADMINS criem múltiplas lojas
            </p>
          </div>
          <button
    ...
    <!-- Geral -->
    <section>
      <h2 class="text-lg font-semibold mb-4">Geral</h2>
    ...
    <section>
      ...
      <p class="text-xs text-gray-500 mb-3">Personaliza a aparência da loja selecionada: <strong>{ctx.currentStore?.name || '-'}</strong></p>
      <form onsubmit={handleBrandingSubmit} class="space-y-5">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">URL da logo</label>
          <input
            type="url"
    ...
    <section>
      ...
      <p class="text-xs text-gray-500 mb-3">Configurações de origem para geração de fretes e etiquetas</p>
      <form onsubmit={handleGeneralSubmit} class="space-y-5">
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">CEP de origem</label>
          <input
            type="text"
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Cidade de origem</label>
          <input
            type="text"
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Endereço de origem</label>
          <input
            type="text"
